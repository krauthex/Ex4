...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node at (2, 0) (nodeL1) {$\ell_1$};
		\node at (2, -1) (nodeL2) {$\ell_2$};
		\node at (2, -2) (nodeL3) {$\ell_3$};
...
\satz{%
	Sei die Spin--Bahnkopplung eines Elektektrons \footnotemark$\ll$ Bahn--Bahn--Kopplung und Spin--Spin--Kopplung zwischen den Elektronen. 
	Dann ist der Gesamtdrehimpuls: 
	$$\vec{J} = \vec{L} + \vec{S} = \sum_i \vec{\ell}_i + \sum_i \vec{s}_i$$
}\footnotetext{\includegraphics[scale=0.4]{elektektron.png} kek!}
...
\bsp{%
	...
	Anzahl der $J$: $\min \lp 2S+1; 2L+1 \rp=3$ \\
	...
	%\begin{figure}[h]
		...
		\end{tikzpicture}
		\captionof{figure}{Vektordiagramme der möglichen Additionen}
	\end{minipage}
	\par\medskip
	 
	\begin{align}
		E_J(n,L,S) &= E(n,L,S) + \frac{C\hslash^2}{2} \lp J(J+1) -8 \rp 
		\intertext{Hier wurde einfach in obige Formel eingesetzt.}
		E_3(n,2,1) &= E(n,2,1) + 2C\hslash^2 \\
		E_2(n,2,1) &= E(n,2,1) - C\hslash^2  \\
		E_1(n,2,1) &= E(n,2,1) -3C\hslash^2  \\
	\end{align}
}
...
Meist ist eine Vielzahl kleinerer Tabellen angegeben. In der linken, oberen Ecke jeder dieser Tabellen steht ''$(x_1)\ \times\ (x_2)$'', also die zwei Beträge der zu addierenden Drehimpulse.\\
